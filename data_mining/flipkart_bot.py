...
def save(data):
    if len(data) > 0:
        pd.DataFrame(data).to_csv('flipkart.cvs', index = False)
    else:
...
def main():
    ...
    datalist = []
    while True:
        url =f'https://www.flipkart.com/search?q=television&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off&as-pos=1&as-type=HISTORY'
        print(url)
        soup = get(url)
        if soup is None:
            break
